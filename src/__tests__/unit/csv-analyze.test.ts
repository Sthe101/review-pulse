  ...
  const rpc = vi.fn(
    async (
      ...
      if (fn === "consume_review_quota") {
        ...
        if (used + n > maxMonth) {
          return {
            data: { ok: false, code: "monthly_review_quota", limit: maxMonth, used },
            error: null,
          };
        }
        if (enforce && (scenario.analysesCountThisMonth ?? 0) >= maxAnalyses) {
          return {
            data: { ok: false, code: "analyses_quota", limit: maxAnalyses },
            error: null,
          };
        }
